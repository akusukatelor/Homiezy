...
<div class="bg-[#F8FAFC] min-h-screen pt-32 pb-20 font-sans italic" 
    ...
    <button 
                @click="step === 5 ? step = 4 : (step === 4 ? step = 2 : window.location.href='{{ route('home') }}')" 
        ...
        <div class="space-y-4">
            ...
            <div class="sticky top-32">
                <div class="bg-white p-10 rounded-[50px] border border-slate-100 shadow-2xl shadow-blue-100 italic">
                 ...
                 <button @click="handleNext()" :disabled="(step === 2 && !kos) || (step === 4 && !laundry)" 
                            ...
                            :class="((step === 2 && !kos) || (step === 4 && !laundry)) ? 'bg-slate-200' : 'bg-[#0095FF] shadow-blue-200'">
    ...
    <div x-show="showSuccessModal" x-cloak
        ...
        <div class="bg-white p-12 rounded-[56px] max-w-lg w-full text-center shadow-2xl transform transition-all"
            ...
            <div class="w-24 h-24 bg-emerald-100 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-8 animate-bounce">
                <i data-lucide="check-circle-2" class="w-12 h-12"></i>
            </div>

            <h2 class="text-4xl font-black text-slate-800 mb-4 italic">Pembayaran Berhasil! 🚀</h2>
            <p class="text-slate-500 font-bold mb-10 text-sm text-center">Terima kasih! Pesanan kamu sudah kami terima.</p>

            <a href="{{ route('home') }}" 
               class="inline-block w-full py-5 bg-[#0095FF] text-white rounded-3xl font-black text-xl shadow-xl hover:bg-blue-600 transition transform hover:scale-105 shadow-blue-200 italic text-center text-sm">
                Kembali ke Beranda
            </a>
        </div>
    </div>
